...
import (
	...
	"github.com/caiquenoboa/banking-auth/domain"
	"github.com/caiquenoboa/banking-auth/dto"
...
func (s DefaultAuthService) Refresh(request dto.RefreshTokenRequest) (*dto.LoginResponse, *errs.AppError) {
	if vErr := request.IsAccessTokenValid(); vErr != nil {
		...
		return nil, errs.NewValidationError("invalid token")
	}
	return nil, errs.NewValidationError("cannot generate a new access token until the current one expires")
}
			...
			*/
			if claims.IsUserRole() {
				if !claims.IsRequestVerifiedWithTokenClaims(urlParams) {
					return errs.NewValidationError("request not verified with the token claims")
				}
			}
			...
			if !isAuthorized {
				return errs.NewValidationError(fmt.Sprintf("%s role is not authorized", claims.Role))
			}
			return nil
		} else {
			return errs.NewValidationError("Invalid token")
		}
	}
}
